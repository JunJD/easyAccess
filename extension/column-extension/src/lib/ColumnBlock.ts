...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    columnBlock: {
      setColumns: (columns: number, keepContent: boolean) => ReturnType;
      ...
      ({ tr, dispatch }: CommandProps) => {
        try {
          ...
          if (firstAncestor === undefined) {
            console.warn('firstAncestor is not find')
            return;
          }
          ...
          console.log('reverse nodes', nodes)
        ...
        try {
          ...
          const canAcceptColumnBlockChild = (par: NodeType) => {
            if (!par.contentMatch.matchType(this.type)) {
              console.log('contentMatch', this.type, par)
              return false;
            }
